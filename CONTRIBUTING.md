...
- **Não use `backdrop-blur` (backdrop-filter).** A NUI do FiveM renderiza fora da tela: não há fundo para desfocar e o efeito vira um retângulo preto maior que o elemento. O template já desativa isso globalmente; para dar contraste, use fundos semitransparentes (`bg-background/90`).
